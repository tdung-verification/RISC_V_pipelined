module datapath(
  ...
  input  logic i_rd_wrenM, i_rd_wrenW, i_lsu_wrenM, i_jalE, i_branchE, i_jalM, i_branchM,
  input  logic i_opb_selE, i_lsu_rdenM, i_alu_decE, i_lsu_rdenW,
  input  logic [1:0] i_opa_selE,
  ...
  output logic [31:0] un_ld_dataW, rd_dataW, ld_dataW, un_ld_dataM,
  ...
  logic [31:0] pcM, pcW, ld_dataM;
  logic insn_vldD, insn_vldE, insn_vldM, insn_vldW;
  logic mispredE, mispredM, mispredW;
  ...
  branch_prediction  prediction(
    ...
    .i_PC_IF(pcF),
	 .i_PC_ID(pcE),
	 .i_PC_EX(pcM),
    .i_br_PC_EX(alu_dataM),
    .i_inst_IF(instF),
    .i_branch_taken_EX(branch_takenM),
    .i_stall(stall),
    .i_is_br_EX(i_branchM),
	 .i_is_jump_EX(i_jalM),
    .o_next_PC(nextpcF),
    .o_flush(flush)
);
  ...
  */
  ...
  //assign operand_a = (i_opa_selE) ? pcE  : data_forward_a;
  ...
  ex_mem flip_flop_EM(
    ...
    .i_alu_dataE(alu_dataE),
	 ...
	 .i_pc_fourE(pc_fourE),
	 .i_pcE(pcE),
	 .i_insn_vldE(insn_vldE),
	 .i_branch_takenE(branch_takenE),
    .o_alu_dataM(alu_dataM),
	 .o_rs2_dataM(rs2_dataM),
	 .o_rd_addrM(rd_addrM),
	 .o_pc_fourM(pc_fourM),
	 .o_insn_vldM(insn_vldM),
	 .o_pcM(pcM),
	 .o_branch_takenM(branch_takenM)
  );
  ...
  assign un_ld_dataM = (i_jalM) ? pc_fourM : alu_dataM;
  ...
  mem_wb flip_flop_mw(
    .i_clk(i_clk),
	 .i_rst_n(i_rst_n),
    .i_ld_dataM(ld_dataM),
	 .i_un_ld_dataM(un_ld_dataM),
	 .i_rd_addrM(rd_addrM),
	 .i_pc_fourM(pc_fourM),
	 .i_insn_vldM(insn_vldM),
	 .i_pcM(pcM),
	 .i_mispredM(mispredM),
    .o_ld_dataW(ld_dataW),
	 .o_un_ld_dataW(un_ld_dataW),
	 .o_rd_addrW(rd_addrW),
	 .o_pc_fourW(pc_fourW),
	 .o_insn_vldW(insn_vldW),
	 .o_pcW(pcW),
	 .o_mispredW(mispredW));
  
  //WB stage
  assign rd_dataW = (i_lsu_rdenW) ? ld_dataW : un_ld_dataW;
  //Hazard
  hazard_unit hazard(
    .Memread(i_lsu_rdenM),
    .Rs1(rs1_addrE),
  ...
  assign mispredM = flush;
  //
  assign o_inst = instD;
  assign o_insn_vld = insn_vldW ;
  assign o_pc_debug = pcW;
  assign o_mispred = mispredW;
